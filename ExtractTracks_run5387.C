#define ExtractTracks_run5387_cxx
#include "ExtractTracks_run5387.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
#include <math.h>
#include "dedx_function_p4.h"


bool ExtractTracks_run5387::IsNotEmptyCheck(Long64_t entry){
...
bool ExtractTracks_run5387::IsStoppingProton(Long64_t entry, TGraph* &graph){
...
bool ExtractTracks_run5387::PassesBeamCuts(Long64_t entry){
...
double ExtractTracks_run5387::Calibrate_dQdx(Long64_t entry, int j, float prim_dqdx, float normalisation_factor, float calib_factor, TH1F* &X_correction_hist_2, TH2F* &YZ_correction_neg_hist_2){
  double prim_hitx=primtrk_hitx->at(0)[j];
  double prim_hity=primtrk_hity->at(0)[j];
  double prim_hitz=primtrk_hitz->at(0)[j];
  double prim_pitch=primtrk_pitch->at(0)[j];
...
void ExtractTracks_run5387::Loop()
{
  //read calibration const -----------------------------------------------//
  vector<double> cali_consts;
  double buffer_cali;
  ifstream f_cali_in("./cali_const_new/cali_consts.txt");
  while (f_cali_in>>buffer_cali) { cali_consts.push_back(buffer_cali); }
  f_cali_in.close();
  double normalisation_factor=cali_consts[0];
  const double calib_factor=(const double)cali_consts[1];
	
  //read calibration parameter -----------------------------------------//
  //X Corr.
  TFile my_file2("./cali_const_new/Xcalo_r5387.root");
  TH1F *X_correction_hist_2 = (TH1F*)my_file2.Get("dqdx_X_correction_hist_2");
  //YZ Corr.
  TFile my_file0("./cali_const_new/YZcalo_r5387.root");
  ...
  // stuff above here probably needed!
  ...
  // output tree                                                         
  TFile* fout = new TFile("output/data_extract_tracks.root", "RECREATE");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) { //evt loop
    ...
    beamline_pdg = pdg_code->at(0);
    std::cout << ientry << " " << beamline_pdg << std::endl;
    ...
    // loop over reco hits of the primary track                  
    for (size_t h=0; h<primtrk_dedx->at(0).size(); ++h) {
      ...
      dedx[h] = dedx_function_p4(prim_dqdx[h],primtrk_hitx->at(0)[h],primtrk_hity->at(0)[h],primtrk_hitz->at(0)[h]);
