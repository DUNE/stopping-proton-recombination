...
// Data Version! - actually just the same as p4???
//double calib_factor =1.029e-3; // the one I had before
//double calib_factor =1.105e-3; // the new one after my calibration a,b
//double calib_factor =1.138e-3; // the 'old' one I calculate
double calib_factor =1.094e-3; // the 'old' one I calculate
//double calib_factor =1.0; // the 'old' one I calculate
double normalisation_factor=0.988;//for plane 2

double Rho = 1.383;//g/cm^3 (liquid argon density at a pressure 18.0 psia) 
double betap = 0.207;//(kV/cm)(g/cm^2)/MeV
double alpha = 0.909;//parameter from ArgoNeuT experiment at 0.481kV/cm 
...
TFile my_file0("/dune/app/users/waldron/dev/2023/April/stopping-protons/cali_box_new/YZcalo_mich2_r5387.root");
TFile my_file2("/dune/app/users/waldron/dev/2023/April/stopping-protons/cali_box_new/Xcalo_mich2_r5387.root");
...
Double_t dedx_function_p4(double dqdx, double x, double y, double z){
 ...
 return corrected_dqdx/((AB/Wion)-((kB/(Rho*Ef))*corrected_dqdx));
}
